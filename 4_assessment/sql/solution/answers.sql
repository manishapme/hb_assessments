...
SELECT m.name, m.brand_name, b.headquarters FROM models AS m JOIN brands AS b ON m.brand_name = b.name WHERE b.name = 'Ford'AND m.name = 'Mustang';
...
SELECT m.name, m.brand_name, b.founded FROM models AS m LEFT JOIN brands AS b ON b.name = m.brand_name WHERE year = 1960;
