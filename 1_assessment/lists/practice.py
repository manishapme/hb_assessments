...
def average(numbers):
    ...
    return float(sum_num) / float(count)
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    For example::
        ...
        >>> reverse_list_in_place([1, 2, 3, 4, 5])
        [5, 4, 3, 2, 1]
    ...
    exit = len(items)-1
    while counter <exit:
        # we will continue appending to the end of the list and removing from the front
        # swapping first and last values is faster. see https://pythonadventures.wordpress.com/2011/06/29/reverse-a-string-or-list-in-place/
        items.extend(items[counter:exit])
        del items[counter:exit]
        counter += 1
...
def duplicates(items):
    ...
    You should do this without changing the original list::
        ...
        ['apple', 'apple', 'berry']
    """
    # by sorting first we know duplicate values will be next to each other
    items.sort()
    duplicates =[]
    ...
    for word in words:
        ...
        while i < len(word):
            if word[i] == letter:
                ...
                break
            i += 1
        indices.append(letter_position)
